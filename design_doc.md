...
> **[SUPERSEDED 2026-08-24]** The delta-log fork was resolved **against** this section. Trigger detection is the performed-action event stream (`GameAction` proposed → CR 614 pipeline → performed `GameEvent`, dispatched synchronously at the mutation instant); no `engine/delta_log.rs` will exist, and the `perform_sba_and_triggers` stub is the *placement* point only. See `plans/codebase-state.md` → "Before Replacement effects" items 3–4 and the Resolution postscript in `plans/atomic-tests/supplemental-docs/state-tracking-architecture.md`.
...
> **[SUPERSEDED 2026-08-24]** Resolved against the delta log — the EventLog is not a projection; it is the retained record of the performed-action stream itself, emitted from the `execute_action`/`move_object` chokepoints (dual-emit fragility is closed by consolidation, not projection). The final paragraph's substance **survives**: trigger matching keys on zone-change events plus last-known characteristics, never on type-specific death events. See `plans/codebase-state.md` → "Before Replacement effects" item 4.

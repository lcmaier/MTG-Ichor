...
use crate::types::effects::{PermanentFilter, TargetSpec, TargetCount};
use crate::types::ids::ObjectId;

impl GameState {
    /// Validate that chosen targets are legal for the given TargetSpec.
    ///
    ...
    pub fn validate_targets(
        ...
        match target_spec {
            ...
            TargetSpec::You => {
                // "You" targeting doesn't use the targets list — the controller is implicit
                Ok(())
            }

            TargetSpec::Creature(count) => {
                self.validate_target_count(count, targets.len())?;
                for t in targets {
                    self.validate_creature_target(t)?;
                }
                Ok(())
            }

            TargetSpec::Player(count) => {
                self.validate_target_count(count, targets.len())?;
                for t in targets {
                    self.validate_player_target(t)?;
                }
                Ok(())
            }

            TargetSpec::Any(count) => {
                self.validate_target_count(count, targets.len())?;
                for t in targets {
                    self.validate_any_target(t)?;
                }
                Ok(())
            }

            TargetSpec::Permanent(filter, count) => {
                self.validate_target_count(count, targets.len())?;
                for t in targets {
                    self.validate_permanent_target(t, filter)?;
                }
                Ok(())
            }

            TargetSpec::Spell(count) => {
                self.validate_target_count(count, targets.len())?;
                for t in targets {
                    self.validate_spell_target(t)?;
                }
                Ok(())
            }
        }
    ...
    pub fn any_targets_still_legal(
        ...
        match target_spec {
            TargetSpec::None | TargetSpec::You => true,
            _ => {
                targets.iter().any(|t| {
                    self.is_single_target_legal(target_spec, t)
                })
            }
        }
    }
    ...
    fn is_single_target_legal(
        ...
        match target_spec {
            TargetSpec::Creature(_) => self.validate_creature_target(target).is_ok(),
            TargetSpec::Player(_) => self.validate_player_target(target).is_ok(),
            TargetSpec::Any(_) => self.validate_any_target(target).is_ok(),
            TargetSpec::Permanent(filter, _) => {
                self.validate_permanent_target(target, filter).is_ok()
            }
            TargetSpec::Spell(_) => self.validate_spell_target(target).is_ok(),
            TargetSpec::None | TargetSpec::You => true,
        }
    }
}
...
mod tests {
    ...
    fn test_validate_permanent_target_all() {
        ...
        let spec = TargetSpec::Permanent(PermanentFilter::All, TargetCount::Exactly(1));
    ...
    fn test_validate_player_target() {
        ...
        let spec = TargetSpec::Player(TargetCount::Exactly(1));
        assert!(game.validate_targets(&spec, &targets).is_ok());
    }
    ...
    fn test_validate_spell_target_not_on_stack() {
        ...
        let spec = TargetSpec::Spell(TargetCount::Exactly(1));
        assert!(game.validate_targets(&spec, &targets).is_err());
    }

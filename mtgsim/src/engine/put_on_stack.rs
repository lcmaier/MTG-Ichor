...
mod tests {
    ...
    fn test_ability_activation_and_resolution_carry_a_durable_identity() {
        ...
        let activated: Vec<_> = game.recorded_events().events().filter_map(|e| match e {
            GameEvent::AbilityActivated { identity, .. } => Some(*identity),
            _ => None,
        }).collect();
        let resolved: Vec<_> = game.recorded_events().events().filter_map(|e| match e {
    ...
    fn make_x_spell() -> std::sync::Arc<crate::objects::card_data::CardData> {
        ...
        CardDataBuilder::new("Blaze")
            ...
            .ability(AbilityDef {
                ...
                costs: Vec::new(),
                effect: Effect::Atom(
                    Primitive::DealDamage { amount: AmountExpr::X, unpreventable: false },

...
pub(crate) use gather::{affected_of, chooser_for, gather, Affected};
pub use gather::CounterEffectKind;
pub(crate) use pipeline::{apply_replacements, Rider};

use crate::state::replacement_effects::ReplacementRowId;
...
///
/// "Once" is per *effect*, so the key has to name the effect and not the row,
/// the object, or the card. Each variant is the identity its source already
/// uses elsewhere in the engine, which is what stops this becoming a fourth
/// notion of identity:
///
/// - a registry row is its `ReplacementRowId`, never reused;

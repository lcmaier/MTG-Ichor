...
/// The CR 613.7 timestamp a row applies under, **read at walk time**.
///
/// CR 613.7a: a static ability's effect "has the same timestamp as the object
/// the static ability is on, or the timestamp of the effect that created the
/// ability, whichever is later" — and clause 3 makes that a live relation:
/// "if the object the ability is on receives a new timestamp, each continuous
/// effect generated by static abilities of that object receives a new
/// timestamp as well." CR 613.7e is the reassignment that makes it matter: an
/// Aura or Equipment gets a new one each time it becomes attached
/// (`GameState::attach`), after `register_static_effects` stamped the row.
///
/// So the stored `ContinuousEffect::timestamp` is the *registry's* key — the
/// object's timestamp at registration, or the grant's — and the key the walk
/// orders by is the later of it and the source's current timestamp. The `max`
/// is exact: the source's timestamp only ever grows, so it is above its own
/// registration value, and a grant later than the source stays the later
/// candidate. A source that is not on the battlefield (a CR 614.12 look-ahead
/// row, whose entity is not inserted yet) has no live value and keeps the
/// stored one. CR 613.7b resolution rows are timestamped once, at creation.
///
/// Read live rather than re-stamped into the rows by `attach` for the reason
/// CLAUDE.md gives for existence: the registry is not reconciled at a
/// state-mutation chokepoint when the walk can read the truth. Measured
/// (`layers-architecture.md` §13a, LH-2): about one percent of CPU/game on
/// the performance pool, inside the sitting's spread.
fn effect_timestamp(game: &GameState, effect: &ContinuousEffect) -> Timestamp {
    match effect.origin {
        EffectOrigin::Resolution => effect.timestamp,
        EffectOrigin::StaticAbility { .. } => game
            .battlefield
            .get(&effect.source)
            .map_or(effect.timestamp, |entity| entity.timestamp.max(effect.timestamp)),
    }
}
...
fn apply_effects(
    ...
    // One scratch buffer for every layer's ordered rows, so a frame allocates
    // once rather than per layer.
    let mut ordered: Vec<(Timestamp, &ContinuousEffect)> = Vec::new();
    ...
    for (layer_index, &layer) in LAYER_ORDER.iter().enumerate() {
        ...
        if has_registered {
            // CR 613.7 order is derived here, not read off the registry: a
            // static ability's row keys on its source's *current* timestamp
            // (613.7a clause 3, 613.7e), which `effect_timestamp` reads live.
            ordered.clear();
            ordered.extend(
                rows_in_layer(game, lookahead, layer, id).map(|e| (effect_timestamp(game, e), e)),
            );
            ordered.sort_by_key(|&(ts, _)| ts);
            for &(timestamp, effect) in ordered.iter() {
                ...
                if !pending_counters.is_empty() && layer == Layer::Layer6Ability {
                    drain_keyword_counters_before(&mut pending_counters, timestamp, chars);
                }

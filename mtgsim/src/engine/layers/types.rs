...
impl EffectModification {
    /// Can applying this modification change *which abilities* an object has?
    ///
    /// Gates the CR 613.7a existence re-check in `compute.rs`: if nothing in
    /// the registry can remove an ability, no static ability can have gone
    /// missing, and the whole check (plus the frame cache it needs) is skipped.
    ///
    /// The match is deliberately exhaustive with no wildcard arm. A new
    /// variant — every Layer 1 copy, Layer 3 text, and Layer 6 ability
    ...
    pub fn can_change_abilities(&self) -> bool {
        match self {
            ...
            // and grants intrinsic mana abilities; adding a basic land type
            // grants one. See `engine/layers/land_types.rs`.
            EffectModification::AddSubtype(_) | EffectModification::SetSubtypes(_) => true,

            // Layer 6 removal. Has an apply arm but no producer yet.
            EffectModification::LoseAllAbilities => true,

            // Conservative. Today's arm is a plain `subtypes.remove` that
            // leaves the frame's abilities alone, so `false` would be
            // behaviourally accurate — but CR 305.6 keys the intrinsic mana
            // ability on the basic land type, so a land losing its last one
            // should lose that ability. Whoever fixes that arm should not also
            ...
            | EffectModification::SetSupertypes(_) => false,

            EffectModification::AddColor(_)
            | EffectModification::SetColors(_)
            | EffectModification::RemoveAllColors => false,

            EffectModification::SetPowerToughness { .. }
            | EffectModification::ModifyPowerToughness { .. }
            | EffectModification::SwitchPowerToughness => false,
        }
    }
}

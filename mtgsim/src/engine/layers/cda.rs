...
//! step; recorded in `codebase-state.md` under the 613.8 item. No
//! tournament-legal card reaches the both-CDA clause (owner search,
//! 2026-09-06), and the dependency loop admits the pair and runs the same
...
mod tests {
    ...
    //
    // Partial: the layer-6 strip is a `ContinuousEffect` written directly into
    // the registry rather than one derived from a real "creatures lose all
    // abilities" card at ETB. That is the right shape for a unit

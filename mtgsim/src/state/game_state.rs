...
impl GameState {
    ...
    pub(crate) fn static_effect_timestamp(
        ...
        match self.battlefield.get(&id) {
            ...
            Some(entry) => entry.timestamp.max(granted_at.unwrap_or(0)),
    ...
    fn register_static_effects(&mut self, id: ObjectId, controller: PlayerId) {
        ...
        for ability in &abilities {
            ...
            for (primitive, recipient) in atoms {
                ...
                // One timestamp for all siblings of one atom (CR 613.1c).
                //
    ...
    ///
    /// Returns several rows only for `ChangeType`, which CR 613.1c splits into
    /// siblings sharing a timestamp and source. An empty `Vec` means the

...
impl ContinuousEffectRegistry {
    ...
    pub fn remove_static_by_source(&mut self, source: ObjectId) -> Vec<Arc<ContinuousEffect>> {
        self.mutating(|rows| {
            rows.retain(|row| {
                !(row.source == source
        ...
        self.mutating(|rows| {
            rows.update_rows(
                |row| {
                    ...
                    row.source == source && is_static && row.timestamp != timestamp
                },
    ...
    pub fn remove(&mut self, id: EffectId) -> Option<Arc<ContinuousEffect>> {
    ...
    pub fn effects_in_layer(&self, layer: Layer) -> &[Arc<ContinuousEffect>] {
...
mod tests {
    ...
    /// A fork shares the rows, so a CR 613.7e re-stamp on the fork copies only
    /// the row it re-stamps, and the original keeps its timestamps.
    #[test]
    fn a_forks_retime_copies_only_the_row_it_writes() {
        let mut reg = ContinuousEffectRegistry::new();
        let (src, other) = (new_object_id(), new_object_id());
        let ability = crate::types::ids::new_ability_id();
        for (source, ts) in [(src, 1), (other, 2)] {
            let mut e = make_effect(source, Layer::Layer6Ability, ts);
            e.origin = EffectOrigin::StaticAbility { ability };
            reg.add(e);
        }
        let mut fork = reg.clone();

        assert_eq!(fork.retime_static_rows(src, 9), 1);

        let stamps = |reg: &ContinuousEffectRegistry| -> Vec<(ObjectId, Timestamp)> {
            reg.iter().map(|e| (e.source, e.timestamp)).collect()
        };
        assert_eq!(stamps(&reg), vec![(src, 1), (other, 2)], "the original is untouched");
        assert_eq!(stamps(&fork), vec![(other, 2), (src, 9)]);
        let (was, now) = (reg.effects_in_layer(Layer::Layer6Ability), fork.effects_in_layer(Layer::Layer6Ability));
        assert!(Arc::ptr_eq(&was[1], &now[0]), "the row it did not write is still shared");
        assert!(!Arc::ptr_eq(&was[0], &now[1]));
    }

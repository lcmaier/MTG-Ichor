...
**Infrastructure added:**
- ~~**Delta log** (`engine/delta_log.rs`)~~ — **STRUCK 2026-08-24: the fork was resolved against the delta log.** Trigger detection is the **performed-action event stream**: `GameAction` (proposed) → CR 614 replacement pipeline → performed `GameEvent` (carrying LKI frame, cause, batch id, resolution context), dispatched **synchronously at the mutation instant** from the `execute_action`/`move_object` chokepoints; state triggers and designations evaluate against live state per atomic event, and only *placement* waits for the CR 603.3 moment. No `engine/delta_log.rs` will exist. Rationale, the four-problem answers, and the refit ticket list: `plans/codebase-state.md` → "Before Replacement effects" items 3–4, and the Resolution postscript in `state-tracking-architecture.md`.
- **Zone-agnostic constraint (survives the strike):** trigger registration must cover objects in ALL zones, not just battlefield permanents. Many paper MTG cards trigger from non-battlefield zones: graveyard (Bloodghast, Narcomoeba), command zone (emblems), exile (suspend). Alchemy boons also trigger from the command zone. The trigger registration index (`HashMap<TriggerKind, Vec<ObjectId>>`) must include objects regardless of zone. See `alchemy-mechanics-audit.md` Q7 for full analysis.
...
> **Architecture change (2026-04-06) — SUPERSEDED 2026-08-24.** The delta-log replacement for the push-based `PendingTrigger` queue was itself resolved against. What survives of its rationale: mutation sites still never know about trigger conditions — they emit performed events into one dispatcher. What changed: detection is synchronous at the mutation instant (CR 702.131d and CR 603.8's momentary conditions require it — the "O(state_triggers × mutations) polling" fear measured out at ~30–40 printed state-trigger cards game-wide, 0–3 per board), and the entries are semantic performed actions, not `(old, new)` field pairs (CR 121.5 makes draw-vs-put a trigger-visible difference no state delta can carry). See `codebase-state.md` → "Before Replacement effects" item 4.
...
- **Two distinct perpetual cost-change patterns:** (1) `SetManaCost` for "mana cost perpetually becomes {X}" (changes mana value), (2) `GrantKeyword` for "perpetually gains 'costs {X} more to cast'" (granted static ability feeding cost pipeline, does NOT change mana value). Incorporate uses pattern 2.
...
> **Disambiguation (2026-08-24):** this block is *diagnostic tooling* (JSON export, snapshots, regression diffing) and is unaffected by the trigger-substrate decision that rejected `engine/delta_log.rs` — but if built, it must be a **projection of the performed-event stream**, never a second source of truth, and nothing game-mechanical may read it. See `codebase-state.md` → "Before Replacement effects" item 4.

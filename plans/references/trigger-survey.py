...
T3 = [
    ...
    ("... the spell it was: mana spent to cast it -- adamant, \"if mana from a Treasure was spent\", the evoke Incarnations", "400.7d / 601.2h",
     ...
     "engine: `CastFacts.mana_spent`, by type (item 30, 2026-09-23); no `Condition` leaf yet. **neither** for a source: \"mana from a Treasure\" and `{S}` need item 33's per-unit pool",
     "12 of these read a source -- add `(o:\"treasure\" or o:/mana from an? / or o:\"{S}\")`; the rest read a type or an amount"),
    ("... the spell it was: kicked or bargained -- an additional cost", "400.7d / 702.33d",
     TRIGGER + ' (o:"was kicked" or o:"was bargained")', ["cast"],
     "engine: `PermanentState.cost_choices` (item 30, 2026-09-23), which a copy of the spell keeps (CR 707.10); `Condition::SpellWasKicked` reads it, off the permanent or the resolving spell; no bargain leaf yet",
     ""),
    ("... the spell it was: an alternative cost -- prowl, surge, spectacle, madness, emerge, sneak", "400.7d / 118.9",
     TRIGGER + ' o:"cost was paid"', ["cast"],
     "engine: `PermanentState.cost_choices` (item 30, 2026-09-23); no `Condition` leaf yet",
     ""),
    ("... the spell it was: cast at all, and by whom", "400.7d / 603.4",
     ...
     "engine: `PermanentState.cast` (`CastFacts`'s `by` and `from`, TR-1); no `Condition` leaf yet",
    ...
    ("... \"for each color of mana spent to cast it\"", "400.7d / 702.44a",
     ...
     "engine: `CastFacts.mana_spent`, by type (item 30); no amount reads it yet", ""),

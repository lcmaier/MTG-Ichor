...
5. ~~**H1, H3–H6** — the latent code fixes~~ ✅ 2026-08-30, branch
   ...
   slice measurement in `codebase-state.md`. **One new row, H10** — CR 101.4's
   APNAP reached neither the CR 704.6d offers nor CR 704.5j's legend groups —
   found while answering an owner question about H4, and closed with it in
   `729b79b`. Theme H now has no `fix` row left; H7–H9 are `defer`/`design`.
6. ~~Then **themes D + F, plus H7–H9, J3–J5 and M1**~~ ✅ 2026-08-30, branch
   `replacement/rb-design`. **Every remaining row is closed.** J1/J2 rode
   along, as the note allowed, and had to: J4's job is re-running the table
   J2 changes. Both fuzz baselines byte-identical, 754 tests, zero warnings,
   `check_claude_md` 199/200, `specdb owed` still 38, `orphans` clean.

   Four things came out of it that were not in the ledger:
   - **M1 was mis-sized.** 44 lines in 14 files, not 94 sites in 33 — the 94
     counted `use crate::types::keywords::KeywordFlag;` imports and the three
     modules that share the word. `TokenDef.keywords` was renamed with the two
     M1 named, having the identical field/type mismatch.
   - **J2's arithmetic did not reproduce, and finding out why found a bigger
     mis-bucket.** Logged as **J6**; it is 21 clauses to J2's 2, and it is what
     actually moved the tier table.
   - **D4 was already answered in the tree**, at `ReplacementInstanceId::Counter`
     — the reviewer was reading `counter_replacements` and had no route to it.
     Closed with a pointer rather than a new argument.
   - **D5 changed shape under measurement.** It asked for a card count and the
     count (~390) is not the blocker; CR 113.6 is, and the layer system needs
     it for the same cards. §11 item 9.
...
**Every row in this file is now closed**, which by the header above makes the
file deletable. Left in place deliberately for the PR that closes it, so the
closures can be read against the rows in one diff; deleting it is the next
session's first line, or the reviewer's.
...
| K3 | §3.3 source 2 (static abilities in other zones) still unsized | §11 item 4 | ✅ **sized 2026-08-30** — `replacement-architecture.md` §11 item 9. ~390 cards, and the blocker is CR 113.6, not the sweep |
...
## D. Scaling — shapes that work at three cards and may not at three thousand — ✅ **CLOSED 2026-08-30**

Not defects. Each is a question about whether the current shape is the one to
grow. **D3 and D4 in `30b4cc7`, D2's comment with them and its entry in
`6efeabf`; D1 and D5 are `replacement-architecture.md` §11 items 8 and 9
(`025acb2`).**

Two came out differently from the row that filed them. **D4's argument already
existed** in full at `ReplacementInstanceId::Counter`, one file away from where
the reviewer was reading — so the close is a cross-reference, not a new
paragraph, and the lesson is that a "this needs explaining" finding should be
checked against the whole type before it is written. **D2's correction was
right and its conclusion was not:** a maintained *set* really would not drift,
but counters have three mutation sites and one of them (CR 704.5q) bypasses the
chokepoint, so the set is unsound for a reason the review had not seen.

| # | Area | Finding | Verdict |
|---|---|---|---|
| D1 | `pipeline.rs:272` | `apply_rewrite`'s match grows per `(template, event)` pair. Is there a shape that does not, or is the hit the right trade? | `design` ✅ — **it does not grow per pair, and the symmetric alternative is worse.** Every template so far reads at most one event kind, so growth is one arm per template plus one error arm; making `GameActionTemplate` an `Option`-field projection of `GameAction` cannot express CR 122.1c turning a `Destroy` into a `RemoveCounters`, and it makes the "your pattern and your rewrite disagree" error unreachable. Tripwire named: the first template built from two event kinds *differently*. §11 item 8 |
| D2 | `gather.rs:344` | `any_replacement_counter` scans every battlefield entity's `counters` map on **every proposed action**. Cheap at 15 permanents and 3 counter kinds; measure before the pool grows. The comment claims a cached count would drift — that is true of a *count*, but not of a set maintained at the same two chokepoints `replacement_ability_sources` uses. | `defer` ✅ — `codebase-state.md` item 22, and the comment corrected in `30b4cc7`. The review's premise holds and its conclusion does not: **counters have three mutation sites, not two**, and CR 704.5q's annihilation writes `BattlefieldEntity` directly (item 6). The set becomes sound the day item 6 lands; measure before paying for it either way |
| D3 | `gather.rs:330` | **`REPLACEMENT_COUNTERS` may be short.** CR 122.1b keyword counters grant keywords, and some keywords are replacement-shaped. Audit all fifteen (122.1b) against CR 614 before trusting the list at three. *(First pass: none of the twelve modelled keyword counters is a CR 614 replacement — indestructible is a "can't", lifelink is CR 120.3f — but that reasoning is nowhere in the code.)* | `fix` ✅ `30b4cc7` — **the list is not short**, and the audit is now at the constant. All nine of CR 122.1's kinds, not just the fifteen keyword counters: a/e/f/g/i are layers, SBA inputs and a trigger. The two near-misses are worth the lines — the rules put indestructible in CR 614.17 and lifelink in CR 120.3f — and vigilance is the third, modifying CR 508.1f's turn-based action rather than replacing an event |
| D4 | `gather.rs:396` | **Why are shield counters two `CounterEffectKind`s when one counter does both?** Because CR 122.1c literally creates two effects with two CR 614.5 identities — but that is not obvious, and the split looks like it defeats the purpose of one counter. Needs the rule quoted at the enum. | `doc` ✅ `30b4cc7` — **the rule was already quoted at the enum and the two-identities argument already written**, at `ReplacementInstanceId::Counter`, which is where the key carrying the variant is defined. The gap was routing: nothing at `counter_replacements` pointed there. Closed with a cross-reference |
| D5 | `gather.rs:11` | **§3.3 source 2 (static abilities functioning in other zones) is deferred past RE — should the phase be able to close without it?** Wonder, Bridge from Below, every "from your graveyard" replacement. A phase called "replacement effects" that cannot see a graveyard source is arguably incomplete. RB was also asked to *size* this and did not (K3). | `design` ✅ §11 item 9 (`025acb2`), closing **K3**. **Yes, it should close without it — and the ~390-card count is not why.** flashback 210, madness 61, buyback 39, rebound 35, aftermath 27, jump-start 13 on the stack and in hand, plus 5 true from-anywhere cards; the 34 disturb DFC hits are *not* source 2 (CR 712.8a). The blocker is **CR 113.6**, which the engine has nowhere and which the **layer system needs for the same cards** — `codebase-state.md` already owes Wonder an object timestamp for exactly this. Building half of it inside RA–RE puts CR 113.6 in `engine/replacement/` |
...
## F. Rules questions that need research before code — ✅ **CLOSED 2026-08-30**

**F2 in `73843d4`; F4 and F6 are `codebase-state.md` items 24 and 25
(`6efeabf`); F1, F3, F5 and F7 are `replacement-architecture.md` §11 items 10,
11, 12 and 13 (`025acb2`).**

**Three of the seven changed size or subject when they were researched.** F1
asked what Skullbriar costs and the answer is that Skullbriar is the wrong
reason — 2 cards want CR 122.2's exception, 71 suspend cards and CR 122.1a/b's
own wording want the capability under it. F3's motivating card turned out to be
a *triggered* ability that never enters the pipeline, and the objection was
still right about the citation. F4 is not a replacement-vocabulary gap at all;
`then` is an `Effect`, which is §3.2's whole point.

| # | Area | Finding | Verdict |
|---|---|---|---|
| F1 | `actions.rs:607` | **Skullbriar, the Walking Grave** — counters stay on it as it changes zones, against CR 122.2. The `AddCounters` performer errors for a non-battlefield object. How much does honouring this change the counter model, and is it worth it? | `design` ✅ §11 item 10 + `codebase-state.md` item 23. **Not worth it for Skullbriar — 2 cards** (it and Me, the Immortal). Worth it for what it needs: counters on an object off the battlefield, which **71 suspend cards** and CR 122.1a/b's own wording want. One move (`counters` → `GameObject`, 12 direct sites), one hard part (`CounterStack.timestamp` means nothing off the battlefield), and Skullbriar then rides along as a predicate at `move_object`'s clear point |
| F2 | `ui/ask.rs:586` | **Partner commanders dying to one board wipe.** The CR 704.6d sweep collects every eligible commander and asks per commander, so it should fire twice — but there is no test, and the UX question (one prompt at a time vs. one combined) is undecided. | `fix` ✅ `73843d4`. It does fire twice. **The UX question is the rule's**: CR 704.6d's "may" is per commander, so two independent choices stay two prompts. Mixed accept/decline is what discriminates, and the test was shown to fail against an offer list deduped by owner |
| F3 | `pipeline.rs:202` | **"If you do" clauses vs. unconditional riders.** RB queues riders unconditionally, citing CR 615.12. That rule is about *prevention*. Academy Rector's "you may exile it. **If you do**, search…" is conditional on its first half actually happening. Is the unconditional rule right for non-prevention replacements? | `design` ✅ §11 item 11. **Right conclusion, wrong citation, wrong card.** Academy Rector is a *triggered* ability and never enters the pipeline (CR 608.2c, §4.1a). But CR 615.12 really is prevention-only, and the `Instead` half rests on CR 614.1a + 614.6 instead. The case separating the two arguments — an `Instead` rider queued, then a later `Prevent` — is named and shown unbuildable in RB. And an "if you do" is `Effect::Conditional`, never a bare `then` |
| F4 | `types/replacement.rs:74` | **Is `then: Option<Effect>` rich enough?** A rider like "you may put a +1/+1 counter on each of them. If you don't, draw a card" needs `Effect::Optional` (unimplemented) plus a conditional on the optional's *outcome* (no vocabulary at all). | `defer` ✅ `codebase-state.md` item 24. **Yes — `then` is an `Effect`, so the gap is the `Effect` tree's and nothing in `types/replacement.rs` moves.** Nor is the demand replacement-shaped: 1,249 cards print "if you do", 165 "if you don't", 35 print "if you do" beside "instead" |
| F5 | `gather.rs:100` | **Should CR 614.15 self-replacement effects be authored or derived?** The plan assumes authored (a `ReplacementClass` a card sets). The analogy to CDAs and CR 605.1 mana abilities suggests *derived* — 614.15's "self-replacement" is a property of where the effect comes from, not a label an author picks. Decide before the first card. | `design` ✅ §11 item 12 — **derived, and the CDA analogy is exact enough to reuse.** `is_characteristic_defining` asserts what an ability's *text* satisfies while CR 604.3a(2)'s provenance belongs to whoever writes the ability onto the object; CR 614.15 is that split with **no text half at all**. So `gather` stamps the class from `ActionContext::resolution` and a card file never writes it — with the reverse assertion too, the analogue of a Layer 6 grant clearing the CDA flag |
| F6 | `actions.rs:282` | **CR 101.4d's restart is not implemented**, and B3 downgrades the claim that it cannot arise. The Notion-Thief-vs-Notion-Thief case (the chooser *changes* as the event is rewritten) **is** handled — `chooser_for` is recomputed every iteration. What is not is interleaving several batch members' choices rather than running each member's loop to completion. | `defer` ✅ `codebase-state.md` item 25. **Sized:** an outer APNAP round-robin driving a per-member loop state machine — a rewrite of `apply_replacements`'s signature, not a patch. Unreachable until two members of one batch can affect each other's choosers, which needs `Retarget` (RD) |
| F7 | `zones.rs:67` | `ZoneChangeCause::Returned` groups "return to hand" and "return to the battlefield". Mechanically distinct despite the shared word; check whether any card distinguishes them as a *cause* before keeping the merge. | `design` ✅ §11 item 13 — **keep the merge, and the merge criterion does not even apply.** The two returns differ by *destination*, which is a field on the event; the enum exists for what `(from, to)` cannot recover. Checked anyway: all 5 cards printing "is returned to" name their zone. The tripwire runs the other way — a card needing "returned to the battlefield" distinct from "put onto the battlefield" at the same `(from, to)` wants a variant on the *put* side |
...
closed 2026-08-30** — H7 as `codebase-state.md` item 26, H8 and H9 as
`replacement-architecture.md` §11 items 14 and 15 (`6efeabf`, `025acb2`); the
theme is now closed entirely. **H10 was found and closed the same day**,
answering an owner question about H4. **No `fix` row is left in this theme.**
Note the line numbers in the rows below predate 2026-08-30's renames and the
H1/H4/H10 fixes; they have drifted.
...
| H10 ✅ | `sba.rs:189` | **CR 704.6d's offers are made in zone-change-epoch order, and CR 101.4 asks for APNAP.** Two commanders in two owners' graveyards in one check produce two prompts to two different players "at the same time", which is 101.4's own condition — "the active player makes any choices required, then each other player in turn order". `moved_since` orders by move, so the owner whose commander died first is asked first regardless of whose turn it is. **Not introduced by H4** (the old sequential loop used the same order) and not a wrong *outcome* — only a wrong ask order, observable at the `DecisionProvider` boundary, which is a UI and AI surface. The fix is one sort on `to_offer` by distance from the active player, the same key `apnap_batch_order` already computes for the batch's replacement choices — the asymmetry is that the batch's CR 616.1 choices are APNAP-ordered and the 704.6d "may" that feeds it is not. Worth checking the same question of the legend-rule prompt (`sba.rs`'s `BTreeMap` groups by `(controller, name)`, so it is controller-index order, not APNAP either). Found 2026-08-30 while answering an owner question about H4. **Closed 2026-08-30 (`729b79b`):** both sites, and the legend rule was mis-ordered too. `GameState::apnap_index` is the shared key — `apnap_batch_order` now uses it as well, so CR 101.4 is written once — and both call sites sort *stably* on it, which is what keeps `moved_since`'s move order and the `BTreeMap`'s name order inside one player. Both tests discriminate only when the active player is not player 0, so both build that. Neither fuzz baseline moved: two players in one legend conflict in one check never came up in 50 games. | `fix` ✅ |
| H7 | `actions.rs:302–312` | **Phase 2 performs do not re-check member legality, and two members about the same object error loudly instead of skipping.** If a batch carries two `ZoneChange`/`Destroy` members naming one object, the first perform moves it and the second errors ("proposed Battlefield→X, which is in Graveyard") — a game-killing `Err` where CR 608.2b's partial resolution says do-as-much-as-possible. Unreachable today: the SBA sweep dedupes per object, combat/untap batches are per-object unique, and no registered spell proposes the same object twice. Becomes reachable with the first multi-clause spell ("Destroy target creature. Destroy target creature.") or any replacement that redirects member 1 into member 2's precondition. Related to F6's interleaving question but distinct: this is perform-phase staleness, not choice-phase restarts. | `defer` ✅ `codebase-state.md` item 26. **Sized:** a `still_legal(&GameAction)` predicate, one arm per variant (10), asked in phase 2 before each perform and skipping rather than erroring — the caller's job by `CLAUDE.md`'s own performers-are-loud rule. What it must *not* be is a `match` on error strings, which would make a real bug indistinguishable from a stale member |
| H8 | `pipeline.rs:108,193` | **Declining CR 903.9b is recorded as final for the whole event, but the rule's 614.5 exemption contemplates re-application after the event changes.** `declined` is keyed on the same instance id regardless of destination, so an owner who declines the command zone for a hand-bound event is never re-asked if a later replacement redirects the event to the library — where 903.9b becomes newly applicable and "may apply more than once to the same event". No second hand/library-redirecting effect exists and nothing sets `is_commander` in real games (K7), so this is unreachable; it is also genuinely unclear in the rules whether a *decline* exhausts the exemption. Research question, not a bug: settle it when RD/RE gives the event a second redirector. | `design` ✅ §11 item 14 — recorded as open, with the constraint an answer has to respect: **`declined` must not be exempted the way `applied` is**, because that exemption is §4.1's termination argument and removing it is a hang. If the answer is "re-ask on a changed destination", the key becomes `(instance, destination)`, still finite |
| H9 | `pipeline.rs:94`, §4.2 | **CR 614.5's identity may need to be per `(event, affected object)`, not per batch member — the shield counter's two-source ruling is the first crack in §4.2's per-member doctrine.** Simultaneous combat damage is one event (CR 510.4); CR 122.1c makes any number of shield counters "a single prevention effect"; the printed ruling removes **one** counter when multiple sources damage the creature at once. The engine models each source's damage as a separate batch member with a fresh applied set, so a creature blocking two attackers with two shield counters loses **two** — each member's prevention applies, each queues a rider. (With one counter the outcome is accidentally right: both prevented, the second rider removes zero and is a no-op.) Kalitas's per-death ruling and this ruling are both real, and they reconcile exactly if the applied set is keyed per affected object within the batch: Kalitas's deaths are different objects → N applications ✓; two damages to one creature are one object → one application ✓. Unreachable today (no shield-counter producer, and it needs a multi-blocked creature), but **RD must settle this before building CR 615.7's shield allocation on the per-member shape** — 615.7's "one shield, several simultaneous sources, controller chooses" is the same modeling question asked officially. | `design` ✅ §11 item 15, **written into the architecture doc rather than left here, because it has a deadline and this file does not outlive its rows.** The reconciliation the row proposes holds, and the cost is smaller than it looks: `apply_replacements` already takes the applied set as a parameter, so only its *owner* changes — `execute_batch_inner` keys one map by affected object instead of cloning per member. §3.2d's `inherited` lineage rule is untouched |
...
## J. The census and the "can't" doc — Q2's answer in rows — ✅ **CLOSED 2026-08-30** (`6b5838a`)
...
**That verdict is half right, and the half that is wrong is the reassuring
half.** 236/2,034 does reproduce — it survives even the corrections, for the
reason §2.1 now gives — but 1,249/1,262 was *not* internally consistent; it was
two hand counts that happened to be added together, and neither reproduces.
Measured, the decomposition is 1,219 + 48 + 10 = **1,277**, and the "2 cards"
global-cap row was never in those units at all. **J6 below is the row this
theme did not have**, and it moved the table four times as far as J1 and J2
together. The apparatus was weaker than the doc implied by more than a rounding
error; the *conclusions* were not, which is the distinction worth keeping.

| # | Area | Finding | Verdict |
|---|---|---|---|
| J1 | `cant-census.py:145` | **"can't search (701.19)" cites the regeneration rule.** Search is **CR 701.23** in `tmnt.txt`; 701.19 is Regenerate — the same number the engine cites correctly a hundred times, so the collision is maximally confusing. Label only; the count is fine. | `fix` ✅ `6b5838a` |
| J2 | `cant-census.py:150` | **"can't be attacked" is bucketed under *attachment* (Tier 2) and is a combat restriction (Tier 1a).** Two clauses hit it — Island Sanctuary and The Aetherspark, verified against the cache — both restrictions on attack declaration. True Tier 2 is 234, Tier 1a 1,264; nothing resizes, but the classifier has a provable mis-bucket and RS-1's spine count inherits it. Nearby, lower: "can't become suspected/night/monarch" (3 clauses) land under "transform / turn face up / phase", which is the right tier with the wrong label. | `fix` ✅ `6b5838a`, **and the predicted numbers do not reproduce.** Only *one* of the two clauses reaches the attachment arm; Island Sanctuary's is caught two arms earlier, by Tier 5 — which is **J6**. The label fix landed as described (the three designations are CR 701.60, 724 and 730) |
| J3 | `cant-effects-architecture.md:142` | **"Every number below is `cant-census.py`" overclaims — §2.4's and §4.2's derived numbers have no script behind them.** The 62 distinct tails, 77 counting clauses, 1,200/49/13 decomposition, the 35%/28%/11% duration shares and the 149/138 conditional counts are all dated hand counts. Two loose threads a reader cannot resolve from the page: §2.4 says 77 counting clauses where §4.2's table has 49 per-attacker counts (where are the other 28?), and the "2 cards" global-cap row only sums to 1,262 if those cards' clauses live inside the 13. Either extend the script to print the decomposition or mark those numbers as hand counts with their method, and reconcile 77/49/13. | `doc` ✅ `6b5838a` — **the script option**, since reconciling required counting anyway. `--decompose` prints the Tier-1a shapes, the tail census and the duration/condition shares. **The 77/49 thread had no resolution and the "2 cards" thread had the answer:** Silent Arbiter and Dueling Grounds print "no more than", never "can't", so they were never among the 2,034 clauses — the row was in the wrong units. Measured: 1,219 / 48 / 10, summing to 1,277 exactly. The 35% share and 149/138 came out of the script *as hand-counted*, which is the argument for dating hand counts rather than deleting them |
| J4 | `cant-effects-architecture.md` §2.1 | The doc's tier table faithfully mirrors the script — which means it inherits J2's ±2. When J2's script fix lands, re-run and update the table (236 → 234, 1,262 → 1,264) in the same commit, or the doc and its evidence disagree by exactly the amount that looks like drift. | `doc` ✅ `6b5838a`, in the same commit as the script, as asked. **Neither predicted number is what happened:** Tier 1a 1,262 → **1,277** and Tier 2 **stays 236**, because its two corrections cancel — one clause out to Tier 1a, one in from Tier 5. Also updated: Tier 1b 122, Tier 3 188, Tier 5 54, residual 6, and `cards-unlocked-ledger.md`'s RS-3a row |
| J5 | `cant-census.py:54` | **The census's query is `o:"can't" -is:funny`, so restrictions phrased "isn't" or "doesn't" are outside all 2,034 clauses** — and that is a *scope*, correctly stated by the script, not a regex bug. It is worth stating in the doc because one such family is not small: **248 cards print "doesn't untap during …"** (`o:/doesn.t untap during/ -is:funny`, 2026-08-30), and 7 print "damage isn't removed …" (`codebase-state.md` item 20). §2.2 "What the census cannot see" already exists for exactly this and currently lists only the keyword-borne restrictions; these are the phrasing-borne ones. Found while answering E4, which is why it is here rather than in E. | `doc` ✅ `6b5838a` — §2.2 gains a phrasing table, and **J3's global caps turned out to be a third row in it**: "no more than N creature can attack" is the same invisibility, and it is why `--decompose` prints those two cards outside its sum |

| J6 | `cant-census.py:106` | **Tier 5's CR 101.3 arm swallowed 21 restrictions, 19 of them Tier 1a** — found 2026-08-30 while checking why J2's arithmetic did not reproduce. The arm matched any clause opening "if you/they …" with a "can't" within 60 characters, which is also the shape of "If you do, target creature can't block this turn" — a conditional *effect* whose payload is a restriction, not CR 101.3's query about whether an instruction can be carried out. One negative lookahead on `do,`/`don't,` fixes it, and Gilded Drake's genuine 101.3 query ("If you don't or can't make an exchange") survives, having no comma after "don't". **This is the correction that moved the tier table**, not J2's: Tier 1a 1,262 → 1,277, Tier 5 74 → 54, Tier 3 +1, residual +2 (the two Lore cards, which are §2.5's "a choice during resolution" shape). Tier 2 unchanged. | `fix` ✅ `6b5838a` |

...
**Shipped.** The mana window and the payer (§3.4, §6): `ManaWindowStop` and `AutoPayer` as two decorators, one per `ChoiceKind`, composed by the client; the sacrifice prompt left the payer on a criterion; the residual 601.2g reading closed both ways. No new card, pool 73. A fourth binary attributed the counter move to the decorator (`codebase-state.md` item 83). No trace page: a read asked by a different party, not answered differently.

→ The section as sized, what the building changed and the measurement: `plans/archive/cost-architecture-landed.md`, "CM-4" (evicted 2026-09-11).

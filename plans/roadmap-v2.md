...
| A6a | **The bounded-state PR** (the owner, 2026-09-25): `codebase-state.md` item 42, no retained log, trigger bindings copying their bound facts at dispatch; item 179, TR-2a's history bounded; the clone probe committed, with CI checks on allocations and bytes per clone; a throwaway probe of the observation cost k, and of a naive redeal (`backlog.md` §2.34) at item 143's checkpoints, with the first decision after it read against a warm and a cold layer memo | floors 2 and 3 (`engineering-practices.md` §3.1) wait on it; before TR-2b, so no later phase builds on a whole-log read or a whole-game history | 1 |
| A6b | ✅ **done 2026-09-26** (PR #193) — **TR-2b**, whose first commit folds the "your" conditions into one variant with `whose: PlayerSet`. **After two fixes, each its own PR.** Item 180's, registry rows shared across a fork: floor 2's CI proxy broke on the budget's own board (✅ 2026-09-25, `codebase-state.md` item 180, archived). Then item 181's, a layers PR: the measurement's brief put a fix ahead of TR-2b if floor 1 broke on a board with a row reaching the hidden zones, and it did (✅ 2026-09-25, LL, `layers-architecture.md` §13e, `codebase-state.md` items 181 and 182, archived) | `triggers-architecture.md` §12, re-counted 2026-09-24; it builds the complete `Primitive::Sacrifice` and the wider `EventKindMask` every later phase reads | 1 |
| A6c | **CV-2, then CV-1b with item 10 (CR 400.7)** | TR-3's returns make new objects; the owner chose the CV order over pulling item 10 forward alone | 2 |
...
| B2 | **`GameConfig::commander()`** and designation, CR 903.7, `DeckLimits` validation | "Before Commander" items 2–3 and main item 32; CR 903.9a/b and 704.6d already work and wait on `is_commander`. **Which cards may be a commander is a format parameter, never the zone's rule** (the owner, 2026-09-26): CR 903.3's legendary creature by default, and a custom format may relax it, a nonlegendary commander for one. The command zone takes whatever the format designates, as `test_support::put_in_command_zone` does | 1 |
...
| B4 | **The information model** (`backlog.md` §2.9) | a v1 blocker, not breadth: the GUI renders one player's view and an AI observation must not leak a hidden zone; **hard back-stop before Phase 8's face-down and reveal cards and before any Phase 10 work**; **its design is A6f (2026-09-25)**, merged with §2.34 and reviewed before any build, and this row is the build; 4 atoms filed in the entry plus CR 400.2. | 1–2 |
...
| B10 | **A clone during a deep stack** — `codebase-state.md` item 183's open half: stack objects shared between forks, copied on write, so a clone copies a pointer per object on the stack | **Hard back-stop before C** (the owner, 2026-09-26): floors 2 and 3 are read between turns, where the stack is empty, and a cEDH line, a storm count or a batch of triggers puts dozens or thousands of objects there mid-turn, where a search clones as well. Phase 8 prints those cards, so the floors hold them first; "possibly sooner", and nothing in A blocks it. The half that fit a clone to what the maps hold after the spike landed with LL's review (`types::ids::FitOnClone`). Sized small: `stack_entries` is written at one `get_mut` and nine inserts, `objects` at three | design + 1 |
...
breadth card: a `cards::helpers` module for the ability constructors and the
duration helper that stamps "X and Y until end of turn" once, the
...
| Item 5b | RS-1–RS-4 | ~4 |
| Item 5c | CV-1–CV-7 | ~7 |
| Item 6 | triggers + LKI (conditional statics landed with LI-3) | 4–6 guessed; **re-counted 2026-09-24 at 12** (TR-1, TR-1b, TR-2a, TR-2b, TR-3a/b, TR-4a/b, TR-5a/b, TR-6, TR-7), `triggers-architecture.md` §12 |

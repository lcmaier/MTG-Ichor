...
| A4 | **RD**, then **RE** (the rest of 5) | RD's CR 120.3 decomposition is what "whenever you lose life" needs to see combat damage; RE's `CreateTokens` is CR 603.2c's "one or more" for 249 token-watching cards and the token-in-exile back-stop ("Before card breadth" item 8), plus `PlayerLoses` and draw replacement so 391 draw-watchers see the post-replacement draw | **RD: 4** (sized 2026-09-08, `replacement-architecture.md` §9) + RE, unsized |

...
| **2. Full-state clone ≤ 10 µs** at item 143's checkpoints | Portable form: ≤ 1/6 of one decision's engine CPU. CI proxy: ≤ 64 allocations per clone | `tests/clone_bound_test.rs`, CI's release step, for the allocations; the readiness pass for the time | **Holds** (the bounded-state PR, 2026-09-25): ≤ 46 allocations, and ≤ 8.5 µs over three sittings of five Commander games, against up to 1,479 and 136 µs with the log |
| **3. ≤ 128 KB per state** | Deep clone size, bounded by the board's high-water mark and never by the turn count | The same test, exact bytes with the hash seed pinned | **Holds** (the same PR): ≤ 107.1 KB at the end of a 168-turn game, the history bounded by the table |

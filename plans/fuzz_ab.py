...
ROWS = [
    ...
    ("Board passes", r"^\s+Board passes:\s+(\d+)"),
...
BOLD = {"Layer walks", "Board passes", "Memo hits", "Layer frames", "Frames/walk", "Replacement gathers", "Restriction queries"}
...
def fmt(name, value):
    if name in ("Layer walks", "Board passes", "Memo hits", "Layer frames") and value.isdigit():
        return f"{int(value):,}"
    return value

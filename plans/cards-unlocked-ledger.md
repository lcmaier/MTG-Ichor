...
| L11 (+ T09) | Control-change effects | Act of Treason ✅ (2026-08-23); Control Magic / Mind Control still need an "enchanted permanent" `ObjectSet` | "Mind Snare" was an invented name — see ATOM-613.1b-001's corpus correction |
| L12 | Text-changing effects | Mind Bend, Sleight of Mind (deferred — infrastructure ready) | |
| L14 | Dependency detection for all layer interactions | Blood Moon + Urborg interaction (L17/L20) | 🃏 **LI-1** (2026-09-06): the board-wide sequential pass (`engine/layers/board.rs`), which fixes Humility + Citanul Hierophants in the pool. 🃏 **LI-2** (2026-09-06): CR 613.8a/b/c — dependencies decided against the live board and re-decided after every application (`layers-architecture.md` §13b); Urborg, Tomb of Yawgmoth registered and pooled, Opalescence and Ashaya, Soul of the Wild registered, the Rootpath Purifier ruling's board as `phase_li_cards::purifier_clause`; the Purifier itself waits on `codebase-state.md` layers item 9 (its library clause). 🃏 **LI-3** (2026-09-06): conditional statics — "as long as [X]" is one more clause in CR 604.2's existence check, evaluated against the live board (`engine/layers/condition.rs`), which closes Deferred item 7f and with it the whole 613.8 cluster. **Kird Ape** registered and pooled — the pool's first row whose existence is a condition; `phase_li_cards::flight_clause` (Rune of Flight's "as long as enchanted permanent is a creature, it has flying") and `phase_li_cards::simian_clause` (a layer-4 condition another layer-4 effect flips) are fixtures registered nowhere. Rune of Flight itself waits on item 6 for its draw trigger *and* on the new item 7g for its Equipment clause |
...
each needing {X}, a condition or a trigger) or an `ObjectSet::Filter` effect,
...
| RE-4 | **CR 614.16's token half, and the vocabulary under it.** The census (Scryfall, 2026-09-11): "would create one or more tokens" **9**, "twice that many … tokens" **10**, "would create … instead" 15, "that many … tokens are created instead" 9, "can't create tokens" 0. **The enabling half is the type**: `backlog.md` §2.27 measured 3,582 printed token producers, 1,085 of which name a token this vocabulary could not say — 749 a CR 111.10 predefined token, 211 an ability quoted inline, 138 "investigate" — and 55 a legendary one; `TokenDef` can now carry all of them, and CR 111.10's twenty predefined tokens are a library away (§2.27, the library half). Every token creation is one proposal now, so a "create" trigger (`GameEvent::TokenCreated`), a CR 101.2 "can't create" and a kind-changing "instead create" each have an event to read | 🃏 **Parallel Lives**, 🃏 **Raise the Alarm** (both `PERFORMANCE_POOL`), 🃏 Hordeling Outburst, 🃏 Hallowed Moonlight, 🃏 Divine Visitation, 🃏 Bard, King of Dale (stress — the last two at the review, which found the kind field and the creation template cheaper to build than to record, and Bard a card both of whose halves were already built). Anointed Procession and Primal Vigor are Parallel Lives' text under another name; Doubling Season's token half is the same and the card is registered whole in RE-5. Ojer Taq's front face is Divine Visitation's shape times three and waits on its back face (CV-5). Blocked on one *later* piece each: Xorn, Chatterfang and Hullbreacher on a Treasure def (`backlog.md` §2.19; their templates exist), Hullbreacher also on a draw-to-creation leg (`codebase-state.md` RE-4 item 126), Jinnie Fay on a creation template with a choice of def (127), every "tapped and attacking" on combat's half of it (128), Academy Manufactor on §2.27's library | ✅ shipped 2026-09-13 |
| RE-6 | **CR 104's two ends as events.** The census is small and the enabling half is not: "would lose the game" **4** (three "if", one "next time"), "would win the game" **0**, "can't lose" **11** and "can't win" **9** (RS's rows, over the two new patterns), and the 38 "you win the game" and 54 "loses the game" effects that are nearly all triggers — item 6's, with `Primitive::{WinGame, LoseGame}` now the thing they resolve into. `Primitive::SetLifeTotal` (CR 119.5) unlocks every "life total becomes N" (Scryfall 2026-09-12: 31 cards) and the three registered rulings that named it. Every state-based loss is a proposal, so a CR 101.2 "can't lose", a "whenever a player loses the game" trigger and a Commander game's third seat all have an event to read | 🃏 **Laboratory Maniac** (`PERFORMANCE_POOL`), 🃏 Exquisite Archangel, 🃏 Stunning Reversal, 🃏 Platinum Angel (all stress). Jace, Wielder of Mysteries is Laboratory Maniac's sentence on a planeswalker; Lich's Mirror waits on `ShuffleIntoLibrary` over three zones, and Gideon of the Trials, Phyrexian Unlife and Angel's Grace are Platinum Angel's row with a condition, a trigger or a duration in front of it. Not unlocked and named: "the game is a draw" (CR 104.4c, no producer), concession (CR 104.3a, no harness), and an each-player draw for Laboratory Maniac's second ruling (item 122) | ✅ shipped 2026-09-12 |

| RE-8 | **CR 701.9's discard and CR 701.22's scry get their producers**, which were two `Primitive` arms returning `Err`. The census (Scryfall, 2026-09-11 and re-run 2026-09-14): "causes you to discard" **17**, sixteen of them "a spell or ability *an opponent controls* causes"; "would discard" **0**, which is why the seventeen watch a zone change rather than a discard event; "would scry" **2**, one a replacement and one a trigger. **The enabling half is what the seventeen and the 1,045 discard-watchers critical-path item 6 counted now have to read**: a discard from a resolution is a `ZoneChange { cause: Discarded }` in the log for the first time, and CR 514.1's cleanup discard is one batch of N rather than N of one, so "whenever one or more cards are discarded" fires once. `ReplacementDef::by` unlocks CR 101.2's cause axis for every replacement effect, not only these — §2.6's Sigarda family had it on the "can't" side alone | 🃏 **Mind Rot**, 🃏 **Opt** (both `PERFORMANCE_POOL`), 🃏 Hymn to Tourach, 🃏 Nephalia Academy, 🃏 Eligeth, Crossroads Augur (stress). **Blocked on CR 113.6 and nothing else — critical-path item 6a**: Dodecapod, Wilt-Leaf Liege, Loxodon Smiter, Nullhide Ferox and Obstinate Baloth, whose clause is on a card in *hand*, and with them the whole printed population of a to-battlefield entry substitution. Blocked on one other facility each: Library of Leng on a maximum hand size (`backlog.md` §2.15), Guerrilla Tactics and Spiritual Focus on critical-path item 6, Coercion on CR 701.9b's third chooser and §2.9's reveal together. Chains of Mephistopheles and the *Words of* discard leg, which RE-2's row named as waiting on this PR, are unblocked on the producer and still want their own kind-changing template | ✅ shipped 2026-09-14 |
...
| Ticket | Cards Unlocked | Example Cards | Status |
|---|---|---|---|
| CV-1 | **Tier C — "becomes a copy" (CR 707.4), turn-bounded.** **56 of 81 clauses**. Also the spine every later CV phase carries: `CopiableValues`, `EffectModification::CopyFrom`, `Primitive::Copy`, and the ETB-scan legs — **three scans, not the two `codebase-state.md` item 16 recorded**; RS-1 had built a third to the same recipe | **Cytoshape** and **Mirrorweave**, both registered (§: Mirrorweave's hybrid cost is simplified to `{2}{W}{U}`, a subset of its legal payments). The two are a pair because `CopyRoles` has two arms and each card is one of them. The Clone-of-an-Anthem probe needed no third card: Cytoshape plus a creature with a static ability reaches it | ✅ **2026-09-02** — 18 tests, both gate legs, `register_copied_static_effects`; `PERFORMANCE_POOL` 62 → 63 |
...
| CV-7 | **Merging (CR 729) + meld (CR 712.4).** 34 + 21 cards, and the multi-component `PermanentState` every other phase writes code against the absence of | Brisela Voice of Nightmares (Bruna + Gisela); Illuna Apex of Wishes and Nethroi Apex of Death (mutate); Urza Lord Protector, which melds | 📋 scheduled 2026-08-30, **undesigned**; **back-stop: before Phase 8 card breadth** |

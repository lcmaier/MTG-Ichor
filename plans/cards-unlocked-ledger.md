...
| L14 | Dependency detection for all layer interactions | Blood Moon + Urborg interaction (L17/L20) | 🃏 **LI-1** (2026-09-06): the board-wide sequential pass (`engine/layers/board.rs`), which fixes Humility + Citanul Hierophants in the pool; the dependency graph is **LI-2** (`layers-architecture.md` §13b) — Urborg, Tomb of Yawgmoth and Ashaya, Soul of the Wild are its cards; Rootpath Purifier waits on `codebase-state.md` layers item 9 (its library clause) |

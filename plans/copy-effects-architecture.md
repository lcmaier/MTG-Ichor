...
### 2.4 The teardown split — 25 clauses out of 150

**What ends a copy row decides the phase boundary, and an earlier draft split
this on the wrong axis.** That draft asked "source-scoped or filter-scoped?" and
answered 133/17. Both the question and the answer were wrong:

- **`AffectedSet` is three-valued, not two**, and CR 611.2c decides which one a
  copy row gets: *"If a continuous effect generated by the resolution of a spell
  or ability modifies the characteristics … the set of objects it affects is
  determined when that continuous effect begins. After that point, the set won't
  change."* So a resolution's copy row is **`AffectedSet::Fixed`** however the
  card is worded — "target creature becomes a copy" and "each other creature
  becomes a copy" both lock their set at resolution. Only a *static* ability
  over a class produces a live `Filter`. The draft's 17 "filter-scoped" clauses
  are almost all `Fixed`.
- **Teardown does not key on `AffectedSet` at all.** It keys on where the row
  came from and how long it lasts.

`copy-census.py --scope`, on the axis that actually maps onto the engine:
...
That is the real CV-1/CV-1b boundary, and it is a third the size of the draft's
but genuinely blocking where the draft's was not.
...
| `ReplacementClass::CopyOnEnter` / `BackFaceUp` | `types/replacement.rs:324,326` | Ship; ordered correctly by `must_choose_among`; **no producer** |
...
- **The capture happens once and is never re-derived.** Two rules, and they
  cover different producers — an earlier draft cited only the second and
  over-generalized it. **CR 707.2b** is the general statement: "Once an object
  has been copied, changing the copiable values of the original object won't
  cause the copy to change." **CR 707.2c** adds the *static-ability* case
  specifically — "the copiable values that effect grants are determined only at
  the time that effect first starts to apply" — and says nothing about
  resolutions. For a resolution the timing comes from **CR 611.2c** instead,
  which locks the affected set when the effect begins, together with CR 707.2's
  definition of copiable values as values read at the moment of copying. All
  three point the same way; only 707.2c is narrow, and citing it alone was the
  draft's error. This is the opposite of how every other continuous effect in
  this engine works — a Layer 6 grant is re-evaluated at every walk.
...
pub struct CopiableValues {
    ...
    /// either — both are capture subjects, so a bare `ManaCost` cannot
    /// represent them.
...
> **An earlier draft answered this wrongly and the error is worth keeping**,
> because it was a *convention* assumed rather than checked. It claimed a Tier C
> copy row has "the copied permanent as **both** source and subject", so
> `remove_by_source` tears it down for free. **It does not.** Every
> resolution-created row in the engine is built with `source: ctx.source`
> (`resolve.rs:318,348,373`), and `ResolutionContext.source` is the *stack
> object* — `stack.rs:112` sets it to the resolving object's id, which for an
> activated ability is `cast.rs`'s ephemeral ability object. A stack object is
> never on the battlefield, so `cleanup_zone_state`'s `remove_by_source` is
> never called with it. The draft's "133 clauses need nothing new" was resting
> on a convention the engine does not have.

**The corrected answer, in three parts** (§2.4's table, and the split is by
origin and duration — not by `AffectedSet`):
...
2. **Tier C turn-bounded, 56 clauses — ships, with the exposure the engine
   ...
   dies and returns *in the same turn* wears the copy again — which is exactly
   the Giant Growth bug item 10 describes, already shipped on every pump spell.
   **Not a reason to block a copy phase**: it is one bug, in one place, that
   item 10 fixes for pumps and copies together.
3. **Tier C indefinite, 25 clauses — this is the real dependency, and it is
   genuinely worse.** Dimir Doppelganger's copy has no duration. The row never
   expires *and* cannot be reached by `remove_by_source`, so it outlives its
   subject **without bound** — and because CR 400.7 preserves `ObjectId`, it
   re-attaches to whatever the returning object is. The asymmetry the draft
...
correct fix is item 10 keyed on the affected object**, which is item 10's own
diagnosis — it "removes only effects *sourced by* the leaving object, never
effects *targeting* it". Copy rows want the second half; so does every pump.
...
prevention effects. A copy row on a permanent is none of those.
...
| `plans/layers-architecture.md:64–68` | the `Layer` code block: `Layer1aFaceDown` cited as "CR 613.2a", `Layer1bCopy` as "CR 613.2b" — both cites attached to the wrong sublayer |
...
**Shipped.** `CopiableValues` captured at `END_OF_LAYER_1` (`engine/layers/copy.rs`), `EffectModification::CopyFrom` applied at `LAYER_ORDER[0]`, `Primitive::Copy(CopyRoles, Duration)`, `ChoiceKind::ChooseCopySource`, both gate legs, `register_copied_static_effects`; Cytoshape (pooled, 62 → 63) and Mirrorweave. Trace page `cv-1-a-copy-is-a-snapshot.html`.
...
4. **Open: does `CopiableValues` want to be `Arc`, not `Box`?** Infinite
   ...
   object. **Not decided here**: it is the 15 class-scoped clauses `--scope`
   prints, the `Box` form is simpler to reason about, and the answer should
   arrive with CV-1's `fuzz_games` measurement rather than before it. Named so
...
- `plans/layers-architecture.md` §7 **and its `Layer` code block at :64–68** —
  the sublayer inversion (§5.4), and a pointer here from §6's provenance table
  for CR 707.9d (§3.4).
- `plans/cant-effects-architecture.md:1281` — its closing "nobody has scheduled
  it" paragraph is now stale in five ways: copy effects have an owning doc, a
  critical-path slot (5c), 101 uncovered atoms not 66, 517 double-faced cards
  not 2,890, and RC-4 is no longer the phase that produces a `CopyOnEnter`.

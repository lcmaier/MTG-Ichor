...
- **`ObjectSet` is three-valued, not two**, and CR 611.2c decides which one a
  ...
  change."* So a resolution's copy row is **`ObjectSet::Fixed`** however the
...
- **Teardown does not key on `ObjectSet` at all.** It keys on where the row
...
> activated ability is `put_on_stack.rs`'s ephemeral ability object. A stack object is
...
| **CV-6 — face-down (CR 708)** | Layer 1b, `PermanentState.face_down`, 708.2a's synthesized 2/2, the CR 708.4 cast-face-down path and the turn-face-up special action. **Consumer: one morph creature, cast and turned up** | **304** producers; **1** new sublayer in `LAYER_ORDER`; the bulk is the *casting* path, not the layer | medium-high — **unsized here on purpose.** The layer half is small and known; the casting half needs its own count of `put_on_stack.rs`'s alternative-cost sites before anyone commits a number |

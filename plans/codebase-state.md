...
10. **CDAs are designed but absent — doc/code drift.** `layers-architecture.md` §6 specifies the whole thing: `ContinuousEffect.is_cda`, `AbilityDef.is_characteristic_defining`, CDA-first partitioning inside each of layers 2–6, and the CR 613.8a(c) short-circuit that makes CDA↔non-CDA pairs independent. It says Phase LA ships the field. **None of it exists in code**, and the `Layer` enum has no `Layer7aCdaPT` — it jumps `Layer6Ability` → `Layer7bSetPT`, so CR 613.4a's sublayer has no home and neither does Tarmogoyf.

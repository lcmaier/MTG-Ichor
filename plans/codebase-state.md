...
7b. **CR 613.7a clause 2 — ✅ implemented (2026-08-23).** "…or the timestamp of the effect that created the ability, whichever is later." It is the `max()` in `GameState::static_effect_timestamp`, which now takes `granted_at: Option<Timestamp>`; `None` means printed, and `register_static_effects` — running at ETB off printed text — is the caller that always passes it. `resolve::register_granted_static_effects` is the clause-2 caller.
    ...
    **One shape does not work, and it asserts rather than failing silently.** A granted static ability whose own effect lands in **layers 1–6** cannot apply: the grant applies *at* layer 6, so at any layer ≤ 6 the frame the CR 613.7a existence check reads is the pre-grant frame, and the derived effect finds no ability to justify itself. This is not a corner — it is CR 613.7a's own worked example, Rune of Flight granting "Equipped creature has flying", which is a layer 6 effect. That card needs Equip as well, so it is out of reach twice over. `register_granted_static_effects` carries a `debug_assert!` at the registration site so a card author is stopped instead of shipping a card that quietly does nothing.
    ...
    Fixing it properly means the grant and the effect it generates cannot be ordered by a single per-object descending walk — it is the same root cause as item 8's CDA↔CDA case, where the unit of ordering has to become an *application* rather than a registry row. Schedule it with that, not before.
...
10. **The `Layer` enum is missing a sublayer split — doc/code drift.** (The CDA half of this item is ✅ done, 2026-08-22; see below.)
    ...
    **Keyword counters carry no timestamp — new, and it is not free.** CR 122.1b keyword counters are applied in Layer 6 from `BattlefieldEntity::counters` (the Layer 7c precedent: derive from state already owned). But CR 613.7c timestamps every counter, and layer 6 is timestamp-ordered, so a keyword counter should interleave with the layer's registry rows — Humility with a later timestamp ought to strip a flying counter's flying. Counters always apply after the slice instead, because `counters: HashMap<CounterType, u32>` has no timestamp to sort by. Layer 7c has the identical gap and it is harmless there because addition commutes; in layer 6 it is not, and `test_keyword_counter_currently_outlasts_a_same_layer_ability_strip` pins the wrong answer so it stays visible.

    Sized: `add_counters` has 17 call sites and `counter_count` 30, and `BattlefieldEntity` cannot allocate a timestamp itself, so the fix is a `GameState`-level wrapper plus a signature change — a counters refactor, not layer work. CR 613.7c's "each counter of that kind receives a new timestamp identical to that of the new counter" means one timestamp per `CounterType` per entity, which is exactly the shape `HashMap<CounterType, (u32, Timestamp)>` already has room for.

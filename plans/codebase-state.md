...
Two follow-ups surfaced by review of PR #59, neither blocking RA-3:

- **`SpellResolved` is a misnomer and always has been** (not new in RA-2). `resolve_top_of_stack` emits it unconditionally, so it fires for activated abilities as well as spells; a trigger matcher keying "whenever a spell resolves" on it would be wrong. Same class as `CreatureDied`/`PlaneswalkerDied` — a type-specific event whose name overpromises — so **fold it into RA-3 ticket 8** rather than treating it separately. Cheap: `ui/display.rs` is the only consumer today.
- **`StackEntry.is_spell` should become a sum type — but at Phase 6 triggers, not in RA-3.** Measured 2026-08-25: 12 construction sites (10 of them test fixtures) and only **2 production readers**, `stack.rs:69` and `stack.rs:180`, both making the same spell-goes-somewhere / ability-evaporates split. The bool plus `cast_from: Option<Zone>` plus `ability_identity: Option<AbilityIdentity>` is honest today, with both invariants asserted. It stops being honest when CR 603 lands: a triggered ability is a third kind of stack object (no cast zone; an identity that is a trigger instance, whose source may already have left the battlefield), at which point `is_spell == false` means "activated *or* triggered" and needs a fourth field to disambiguate. Do it as the first commit of the triggers phase, where there is a forcing function and a third variant to design against — not speculatively now.

...
51. **A rewound cast leaves its CR 601.2a move in the log.** `cast_spell`
    ...
    after cost payment begins, so nothing paid is ever un-paid. **Sized:**
    ~30 lines in `cast.rs`; do it with RC-4b or with item 6, whichever comes
    first. The rule both fixes follow is `replacement-architecture.md` §11
    item 20.

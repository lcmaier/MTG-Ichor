...
46. **The frame is per entry, not per batch, and §5b says it should be per
    ...
    `perform_action`'s `ZoneChange` arm.
...
4. **An ETB trigger keys on `PermanentEnteredBattlefield`, never on `ZoneChange { to: Battlefield }` (recorded 2026-09-02, RC-4).** Containment Priest's "exile it instead" rewrites the `EnterBattlefield` into a `ZoneChange` out of the zone, so the log holds a zone change *into* the battlefield for a card that never became a permanent (RC-2's one-`emit`-wide window, made observable). CR 603.6a's "enters the battlefield" is the performer's event, and it is the only event a permanent that never entered does not have. The trigger matcher must read that one; a matcher on the zone change would fire Priest-exiled entries.

...
7. **`register_static_effects` reads printed abilities — ⚠️ known gap.** `state/game_state.rs` scans `obj.card_data.abilities` at ETB. It cannot use the layer system: it runs inside `place_on_battlefield`, before the object's own effect is registered, so `compute_characteristics` there is circular. Two consequences, both blocking Layer 6. Re-registration must follow CR 613.7a: a continuous effect generated by a static ability takes the timestamp of the object the ability is on, or of the effect that granted the ability, whichever is later.
   - A static ability *granted* to a permanent never registers its continuous effect.
   - A static ability *stripped* from a permanent (Blood Moon on a land with one) keeps its already-registered effect applying. Nothing re-checks whether the source still has the ability.

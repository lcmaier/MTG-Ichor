// src/bin/casting_spells_demo.rs
use std::io;
use std::io::Write;
use uuid::Uuid;

use mtgsim::game::player::Player;
use mtgsim::utils::constants::abilities::{AbilityType, EffectDetails};
use mtgsim::utils::constants::events::{EventHandler, GameEvent};
use mtgsim::utils::constants::game_objects::{BattlefieldState, GameObj, HandState};
use mtgsim::game::gamestate::Game;
use mtgsim::utils::constants::card_types::CardType;
use mtgsim::utils::constants::deck::{Deck, DeckFormat};
use mtgsim::utils::constants::turns::PhaseType;
use mtgsim::utils::constants::zones::Zone;
use mtgsim::utils::constants::id_types::PlayerId;
use mtgsim::utils::targeting::core::{TargetCategory, TargetCriteria, TargetRef};
use mtgsim::cards::generator::ObjectGenerator;
use mtgsim::cards::basic_lands::{create_basic_land, BasicLand};

// Create a test deck with lands and Lightning Bolts
fn create_test_deck_with_spells(owner: PlayerId) -> Deck {
    let mut deck = Deck::new(owner, DeckFormat::Limited);

    // Add 10 Mountains, 10 Forests
    for _ in 0..10 {
        deck.put_card_on_bottom(create_basic_land(BasicLand::Mountain, owner));
        deck.put_card_on_bottom(create_basic_land(BasicLand::Forest, owner));
    }

    // Add 20 Lightning Bolts
    for _ in 0..20 {
        match ObjectGenerator::create_card_in_library("Lightning Bolt", owner) {
            Ok(card) => deck.put_card_on_bottom(card),
            Err(e) => {
                println!("Error creating Lightning Bolt: {}", e);
                // Continue anyway for resilience
            }
        }
    }

    deck.shuffle();
    deck
}

// The main casting UI flow
fn handle_cast_spell(game: &mut Game, player_id: PlayerId) -> Result<(), String> {
    // 601.2a - Propose the spell
    let card_id = propose_spell(game, player_id)?;
    
    // 601.2b - Choose modes, alternative costs, etc.
    // (not yet implemented)
    // let mode_choices = choose_spell_modes(game, player_id, card_id)?;
    
    // 601.2c - Choose targets
    let targets = choose_spell_targets(game, player_id, card_id)?;
    
    // 3. Pass everything to the game engine to handle rules and payment
    game.cast_spell(player_id, card_id, Zone::Hand, targets)?;
    Ok(())
}

// Step 1: Propose the spell (601.2a)
fn propose_spell(game: &Game, player_id: PlayerId) -> Result<Uuid, String> {
    let player = game.get_player_ref(player_id)?;
    // get cards castable from hand
    let spell_cards: Vec<(usize, &GameObj<HandState>)> = player.hand.iter()
        .enumerate()
        .filter(|(_, card)| !card.has_card_type(&CardType::Land))
        .collect();

    // TODO: get cards castable from other zones (e.g. graveyard, exile) if applicable
    if spell_cards.is_empty() {
        return Err("No spells in hand to cast.".to_string());
    }

    println!("Select a spell to cast:");
    for (i, (_, card)) in spell_cards.iter().enumerate() {
        if let (Some(name), Some(rules_text)) = (&card.characteristics.name, &card.characteristics.rules_text) {
            println!("{}: {} - {}", i + 1, name, rules_text);
        }
    }

    let mut selection = String::new();
    print!("> ");
    io::stdout().flush().unwrap();
    io::stdin().read_line(&mut selection).unwrap();

    let selected_index = match selection.trim().parse::<usize>() {
        Ok(i) if i > 0 && i <= spell_cards.len() => i - 1,
        _ => return Err("Invalid selection".to_string()),
    };

    let (_, card) = spell_cards[selected_index];
    Ok(card.id)
}

// Step 3: Choose targets (601.2c)
fn choose_spell_targets(game: &Game, player_id: PlayerId, card_id: Uuid) -> Result<Vec<TargetRef>, String> {
    let player = game.get_player_ref(player_id)?;
    let card = player.get_card_in_hand(card_id)
        .ok_or_else(|| "Card not found in hand".to_string())?;
    // Check if we need to get targets
    if let Some(abilities) = &card.characteristics.abilities {
        for ability in abilities {
            match ability.ability_type {
                AbilityType::Spell => { 
                    match &ability.effect_details {
                        EffectDetails::DealDamage { amount: _, target_requirement } => {
                            if let Some(requirement) = target_requirement {
                                // Targets we need to deal with
                                match &requirement.criteria {
                                    TargetCriteria::Category(category) => {
                                        match category {
                                            TargetCategory::AnyDamageable => { // Single Target, any Damageable (e.g. Lightning Bolt)
                                                let mut target_options = Vec::new();

                                                // add players
                                                for player_id in 0..game.players.len() {
                                                    println!("{}: Player {}", target_options.len() + 1, player_id);
                                                    target_options.push(TargetRef::player(player_id));
                                                }

                                                // add damageable permanents
                                                for (_, permanent) in game.battlefield.iter() {
                                                    if permanent.has_card_type(&CardType::Creature) ||
                                                       permanent.has_card_type(&CardType::Planeswalker) ||
                                                       permanent.has_card_type(&CardType::Battle) {
                                                        if let Some(name) = &permanent.characteristics.name {
                                                            println!("{}: {} (on battlefield)", target_options.len() + 1, name);
                                                            target_options.push(TargetRef::permanent(permanent.id));
                                                        }
                                                    }   
                                                }

                                                if target_options.is_empty() {
                                                    println!("No valid targets for {}", card.characteristics.name.clone().unwrap());
                                                    continue;
                                                }
                                                // Get target selection
                                                println!("Select a target:");
                                                let mut target_selection = String::new();
                                                print!("> ");
                                                io::stdout().flush().unwrap();
                                                io::stdin().read_line(&mut target_selection).unwrap();

                                                let target_index = match target_selection.trim().parse::<usize>() {
                                                    Ok(i) if i > 0 && i <= target_options.len() => i - 1,
                                                    _ => {
                                                        println!("Invalid target selection");
                                                        continue;
                                                    }
                                                };

                                                let selected_target = target_options[target_index].clone();                        
                                                // return the targets, if any
                                                let mut targets = Vec::new();
                                                targets.push(selected_target);
                                                return Ok(targets)
                                            },
                                            _ => println!("Lightning Bolt only card currently implemented")
                                        }
                                    },
                                    _ => println!("Lightning Bolt only card currently implemented")
                                }
                            }
                        },
                        _ => println!("Lightning Bolt only card currently implemented")
                    }
                },
                _ => continue, // we only care about spell ability targets when we're casting the spell
            }
        }
    }
    
    // Default for spells with no targets
    Ok(Vec::new())
}

fn main() {
    println!("MTG Simulator - Casting Spells Demo");

    // Create a new game
    let mut game = Game::new();

    // Create two players
    let mut pid_counter = 0;
    let player1_id = pid_counter;
    let mut player1 = Player::new(player1_id, 20, 7, 1);
    pid_counter += 1;

    let player2_id = pid_counter;
    let mut player2 = Player::new(player2_id, 20, 7, 1);

    // Create decks with spells
    let deck1 = create_test_deck_with_spells(player1.id);
    let deck2 = create_test_deck_with_spells(player2.id);

    println!("Created decks with {} cards each", deck1.size());

    // Set the player's libraries
    player1.set_library(deck1.cards.clone());
    player2.set_library(deck2.cards.clone());

    // Add the players to the game
    game.players.push(player1);
    game.players.push(player2);

    // Have both players draw a starting hand
    for player_id in [player1_id, player2_id] {
        let player = game.get_player_mut(player_id).unwrap();
        match player.draw_n_cards(player.max_hand_size as u64) {
            Ok(_) => println!("Player {} drew {} cards.", player.id, player.max_hand_size),
            Err(e) => {
                println!("Error drawing cards: {}", e);
                return;
            }
        }
    }

    // Process initial phase and step
    match game.process_current_phase() {
        Ok(_) => println!("Initial phase/step processed."),
        Err(e) => {
            println!("Error processing initial phase: {}", e);
            return;
        }
    }

    // Game loop
    loop {
        // get turn player and phase info
        let turn_player_id = game.active_player_id;

        println!("\n=== Player {}'s Turn ===", turn_player_id);
        println!("Turn Number: {}", game.turn_number);
        println!("Phase: {:?}", game.phase.phase_type);

        if let Some(ref step) = game.phase.current_step {
            println!("Step: {:?}", step.step_type);
        } else {
            println!("No current step (Main Phase)");
        }

        // Display mana pool for both players
        for player_id in [player1_id, player2_id] {
            let player = game.get_player_ref(player_id).unwrap();
            println!("Player {}'s mana pool:", player_id);
            let mana_pool = player.mana_pool.get_available_mana();
            if mana_pool.is_empty() {
                println!("  (Empty)");
            } else {
                for (mana_type, amount) in mana_pool {
                    println!("  {:?}: {}", mana_type, amount);
                }
            }
        }

        // Display life totals for both players
        for player_id in [player1_id, player2_id] {
            let player = game.get_player_ref(player_id).unwrap();
            println!("Player {}'s life total: {}", player_id, player.life_total);
        }

        if game.priority_player_id == turn_player_id {
            // Display active player's hand 
            let active_player = game.get_player_ref(game.active_player_id).unwrap();

            println!("\nYour hand:");
            for (i, card) in active_player.hand.iter().enumerate() {
                if let Some(name) = &card.characteristics.name {
                    if let Some(rules_text) = &card.characteristics.rules_text {
                        println!("{}: {} - {}", i + 1, name, rules_text);
                    } else {
                        println!("{}: {} - (No rules text)", i + 1, name);
                    }
                } else {
                    println!("{}: Unknown card", i + 1);
                }
            }

            // Display the battlefield
            println!("\nBattlefield:");
            if game.battlefield.is_empty() {
                println!("(Empty)");
            } else {
                for (i, (_, card)) in game.battlefield.iter().enumerate() {
                    if let Some(name) = &card.characteristics.name {
                        if let Some(rules_text) = &card.characteristics.rules_text {
                            println!("{}: {} - {} (Owner: {})", i + 1, name, rules_text, card.owner);
                        } else {
                            println!("{}: {} - (No rules text) (Owner: {})", i + 1, name, card.owner);
                        }
                    } else {
                        println!("{}: Unknown card (Owner: {})", i + 1, card.owner);
                    }
                }
            }

            // Player options
            println!("\nWhat would you like to do?");
                
            // Only show play land option during main phases
            if game.phase.phase_type == PhaseType::Precombat || game.phase.phase_type == PhaseType::Postcombat {
                println!("1. Play a land");
            }
            
            println!("2. Tap a land for mana");
            println!("3. Cast a spell");
            println!("4. Pass priority");
            println!("5. Quit");

            let mut choice = String::new();
            print!("> ");
            io::stdout().flush().unwrap();
            io::stdin().read_line(&mut choice).unwrap();

            match choice.trim() {
                "1" if game.phase.phase_type == PhaseType::Precombat || game.phase.phase_type == PhaseType::Postcombat => {
                    // Land playing code (same as the turn_structure_demo)
                    let player = game.get_player_ref(turn_player_id).unwrap();
                    let land_cards: Vec<(usize, &GameObj<HandState>)> = player.hand.iter()
                        .enumerate()
                        .filter(|(_, card)| card.has_card_type(&CardType::Land))
                        .collect();

                    // Ensure there are lands to show
                    if land_cards.is_empty() {
                        println!("No lands in hand to play.");
                        continue;
                    }

                    // Show lands to the user
                    println!("Select a land to play:");
                    for (i, (_, card)) in land_cards.iter().enumerate() {
                        if let (Some(name), Some(rules_text)) = (&card.characteristics.name, &card.characteristics.rules_text) {
                            println!("{}: {} - {}", i + 1, name, rules_text);
                        }
                    }

                    // Get user selection
                    let mut selection = String::new();
                    print!("> ");
                    io::stdout().flush().unwrap();
                    io::stdin().read_line(&mut selection).unwrap();

                    // Process selection
                    let selected_index = match selection.trim().parse::<usize>() {
                        Ok(i) if i > 0 && i <= land_cards.len() => i - 1,
                        _ => {
                            println!("Invalid selection");
                            continue;
                        }
                    };

                    // Get the card_id of selected card
                    let (_, card) = land_cards[selected_index];
                    let card_id = card.id;

                    // Play the land
                    match game.play_land_from_hand(turn_player_id, card_id) {
                        Ok(_) => println!("Land played successfully!"),
                        Err(e) => println!("Error playing land: {}", e),
                    }
                },
                "2" => {
                    // Tap land for mana (same as before)
                    let untapped_lands: Vec<(usize, &GameObj<BattlefieldState>)> = game.battlefield.values()
                        .enumerate()
                        .filter(|(_, card)| {
                            card.has_card_type(&CardType::Land) &&
                            card.state.controller == game.priority_player_id &&
                            !card.state.tapped
                        })
                        .collect();
                    
                    if untapped_lands.is_empty() {
                        println!("You control no untapped lands.");
                        continue;
                    }

                    println!("Select a land to tap for mana:");
                    for (i, (_, card)) in untapped_lands.iter().enumerate() {
                        if let Some(name) = &card.characteristics.name {
                            println!("{}: {}", i + 1, name);
                        } else {
                            println!("{}: Unknown land", i + 1);
                        }
                    }

                    let mut selection = String::new();
                    print!("> ");
                    io::stdout().flush().unwrap();
                    io::stdin().read_line(&mut selection).unwrap();

                    let selected_index = match selection.trim().parse::<usize>() {
                        Ok(i) if i > 0 && i <= untapped_lands.len() => i - 1,
                        _ => {
                            println!("Invalid selection");
                            continue;
                        }
                    };

                    let (_, land) = untapped_lands[selected_index];
                    let land_id = land.id;

                    if let Some(abilities) = &land.characteristics.abilities {
                        if let Some(_) = abilities.iter()
                            .find(|ability| ability.ability_type == AbilityType::Mana) {
                            let mana_activation_event = GameEvent::ManaAbilityActivated { 
                                source_id: land_id, 
                                player_id: game.priority_player_id 
                            };

                            match game.handle_event(&mana_activation_event) {
                                Ok(_) => println!("Mana ability activated successfully!"),
                                Err(e) => println!("Error activating mana ability: {}", e),
                            }
                        } else {
                            println!("Selected land has no mana ability.");
                        }
                    } else {
                        println!("Selected land has no abilities.");
                    }
                },
                "3" => {
                    // Cast a spell using our structured approach
                    match handle_cast_spell(&mut game, turn_player_id) {
                        Ok(_) => {},
                        Err(e) => println!("Error casting spell: {}", e),
                    }
                },
                "4" => {
                    // Pass priority
                    println!("Passing priority...");
                    match game.pass_priority() {
                        Ok(phase_changed) => {
                            if phase_changed {
                                println!("Game Event resolved");
                            }
                        },
                        Err(e) => println!("Error passing priority: {}", e),
                    }
                },
                "5" => {
                    println!("Goodbye!");
                    break;
                },
                _ => println!("Invalid choice"),
            }
        } else {
            // Non-active player's turn or active player doesn't have priority
            println!("\nWaiting for Player {} to act...", game.priority_player_id);
            println!("2. Tap a land for mana");
            println!("3. Cast a spell");
            println!("4. Pass priority");
            println!("5. Quit");

            let mut selection = String::new();
            print!("> ");
            io::stdout().flush().unwrap();
            io::stdin().read_line(&mut selection).unwrap();

            // Handle non-active player actions similar to active player
            match selection.trim() {
                "2" => {
                    // Tap land for mana (same as before)
                    let untapped_lands: Vec<(usize, &GameObj<BattlefieldState>)> = game.battlefield.values()
                        .enumerate()
                        .filter(|(_, card)| {
                            card.has_card_type(&CardType::Land) &&
                            card.state.controller == game.priority_player_id &&
                            !card.state.tapped
                        })
                        .collect();
                    
                    if untapped_lands.is_empty() {
                        println!("You control no untapped lands.");
                        continue;
                    }

                    println!("Select a land to tap for mana:");
                    for (i, (_, card)) in untapped_lands.iter().enumerate() {
                        if let Some(name) = &card.characteristics.name {
                            println!("{}: {}", i + 1, name);
                        } else {
                            println!("{}: Unknown land", i + 1);
                        }
                    }

                    let mut selection = String::new();
                    print!("> ");
                    io::stdout().flush().unwrap();
                    io::stdin().read_line(&mut selection).unwrap();

                    let selected_index = match selection.trim().parse::<usize>() {
                        Ok(i) if i > 0 && i <= untapped_lands.len() => i - 1,
                        _ => {
                            println!("Invalid selection");
                            continue;
                        }
                    };

                    let (_, land) = untapped_lands[selected_index];
                    let land_id = land.id;

                    if let Some(abilities) = &land.characteristics.abilities {
                        if let Some(_) = abilities.iter()
                            .find(|ability| ability.ability_type == AbilityType::Mana) {
                            let mana_activation_event = GameEvent::ManaAbilityActivated { 
                                source_id: land_id, 
                                player_id: game.priority_player_id 
                            };

                            match game.handle_event(&mana_activation_event) {
                                Ok(_) => println!("Mana ability activated successfully!"),
                                Err(e) => println!("Error activating mana ability: {}", e),
                            }
                        } else {
                            println!("Selected land has no mana ability.");
                        }
                    } else {
                        println!("Selected land has no abilities.");
                    }
                },
                "3" => {
                    // Cast a spell
                    let priority_player = game.priority_player_id; // need to extract this for the borrow checker
                    match handle_cast_spell(&mut game, priority_player) {
                        Ok(_) => {},
                        Err(e) => println!("Error casting spell: {}", e),
                    }
                },
                "4" => {
                    // Pass priority
                    println!("Passing priority...");
                    match game.pass_priority() {
                        Ok(phase_changed) => {
                            if phase_changed {
                                println!("Phase or step advanced");
                            }
                        },
                        Err(e) => println!("Error passing priority: {}", e),
                    }
                },
                "5" => {
                    println!("Goodbye!");
                    break;
                },
                _ => println!("Invalid choice"),
            }
        }
    }
}
...
python plans/check_claude_md.py && python plans/check_module_layout.py && python plans/check_glossary.py && python plans/check_state_of_play.py --check && python plans/check_rulings.py --check && python plans/check_string_literals.py && python plans/check_type_surface.py && python plans/check_clippy_allows.py   # all must pass
```
...
| `plans/engineering-practices.md` | Process: this file's budget, the comment rule, the two card pools, phase sizing, the specdb gate, module layout, trace pages (§7), **the rules pass (§8)**. §3 owns the measuring rules; the measurements are `plans/fuzz-record.md`, newest-first, a block per phase |

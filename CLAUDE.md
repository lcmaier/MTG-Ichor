...
| `plans/replacement-architecture.md` | Replacement + prevention (CR 614–616): the event vocabulary, the CR 616.1 pipeline, the ETB look-ahead frame, phase sequencing |
...
| `plans/references/*` | External tooling reference, not rules authority. `scryfall-syntax.md` is the search-filter cheat sheet; `replacement-census.py` re-runs the card-pool clause census behind `replacement-architecture.md` §3.2c. Fetch card examples/rulings via Bash+curl with a UA header — Scryfall 403s the `WebFetch` tool. These are **research tools**: they print numbers a human pastes into prose, and generate no file the project reads |

Generated by `python plans/specdb.py build`, all from `sessions/`, never hand-edit —
fix the session file and rebuild: `spec.sqlite`, `global-test-index.md`,
`phase-index-*.md`.

`atomic-tests/summaries/` is the authoring trail, not a source: it generated the
indexes until 2026-08-24 and drifted from the corpus by 27 entries. Nothing reads it.
...
5. Replacement effects (CR 614–616) — stub hook in `execute_action`. Gates most real
   cards *and* Commander's 903.9b command-zone redirection. Execute from
   `plans/replacement-architecture.md` (phases RA–RE)
...
track — cost modification (commander tax), 903.9a/b, `GameConfig::commander()`, CR 800
...
python plans/specdb.py suspicious               # annotations that look mis-linked
```
...
blank. `orphans` catches an id that doesn't exist; `suspicious` catches one that exists
and looks mis-linked, by comparing the atom's scenario vocabulary against the test's. It
is a smell test — a hit means read it, silence does not mean the links are right. Tests with no matching atom are normal; this measures rules coverage, not project

...
| `plans/references/*` | External tooling reference, not rules authority. `scryfall-syntax.md` is the search-filter cheat sheet; `replacement-census.py` re-runs the card-pool censuses behind `replacement-architecture.md` §3.2c/§8b. Fetch card examples/rulings via Bash+curl with a UA header — Scryfall 403s the `WebFetch` tool. These are **research tools**: they print numbers a human pastes into prose, and generate no file the project reads |
